...
import "./add_form.css";
import NewInput from "../../components/new_inputs/newInput";
import Existing from "../../components/existing_inputs/existing";
import New from "../../components/new_inputs/new";
import { MdDelete } from "react-icons/md";
import { RxCross2 } from "react-icons/rx";
import { FiPlus } from "react-icons/fi";

function Add_form() {
    const [add_form, setAddForm] = useState([
        <FiPlus className="icon_align" />,
        true,
    ]);
    ...
    const Delete_input = (inputToDelete) => {
        sinputfunc(
            (prevArray) => prevArray.filter((item) => item !== inputToDelete) // Remove the element with the given value
        );
    };
    ...
    return (
        <div className="Add_Form">
            ...
            <div>
                ...
                {showexist && (
                    <Existing
                        exist_click={handleCancel}
                        save_input={handleSave}
                        exist_input={exist_input}
                    />
                )}
                {shownew && (
                    <New
                        Cancel_click={handleCancel}
                        Submit_Click={handleSave}
                    />
                )}
            </div>
